...
Capturer::Capturer(HCore::HCore *core)
    : Threadable(Settings::capture.rate, "Capturer", false, core), offsetX(0),
      offsetY(0) {
    m_limit = false;
...
void Capturer::preprocessFramebuffer(Framebuffer &fb) {
    ...
    if (Settings::detection.aroundMouse) {
        ...
        Settings::detection.x =
            (std::max)(0, pos.x - Settings::detection.w / 2);
        Settings::detection.y =
            (std::max)(0, pos.y - Settings::detection.h / 2);
    }
    ...
    if (fb.fWidth != Settings::net.w || fb.fHeight != Settings::net.h ||
        ...
        fb.resize(fb.capture.w, fb.capture.h, Settings::net.w, Settings::net.h,
                  ARGB);
    }
}
...
void Capturer::processFramebuffer(Framebuffer &fb, unsigned char *buffer) {
    ...
    for (int i = 0; i < fb.detection.h; i++) {
        tmp[i] = &buffer[(
            int)((i + fb.detection.y - fb.capture.y) * fb.width *
                     fb.bytesPerPixel +
    ...
    int threads = (std::max)(1, fb.fHeight / 8);

    for (int i = 0; i < threads; i++) {
        int start = (int)(i * ((float)fb.fHeight / threads));
        int end = (int)((float)fb.fHeight / threads * (i + 1));
        m_core->tPool->enqueueBlocking(threadKey, &Framebuffer::scaleRows, fb,
                                       start, end, fb.detection.w,
                                       fb.detection.h, tmp);
...
void Capturer::work() {
    ...
    /*
    /if(Settings::capture.format.compare("RGB")){
        printf("FOrm");
