...
// Capturer handles filling these and does so more often than other workers need
// them.
...
Framebuffer &getFramebuffer(uint32_t index) { return m_framebuffers.at(index); }
...
void finishCapture(int id) { m_indexDetect = id; }
void finishDetect(int id) { m_indexPresent = id; }
