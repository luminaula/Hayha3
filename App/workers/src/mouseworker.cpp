...
MouseWorker::MouseWorker(HCore::HCore *core):
        Threadable(Settings::mouse.rate,"MouseWorker",false,core){
    ...
    if(!m_mouseQueue.empty()){
        ...
        switch(node.code){
            ...
            case MOUSE_MOVE:
                accumX = node.x;
                accumY = node.y;
                while(!m_mouseQueue.empty() && m_mouseQueue.front().code == MOUSE_MOVE){
                    node = m_mouseQueue.front();
                    m_mouseQueue.pop();
                    accumX += node.x;
                    accumY += node.y;
                }
