...
HDetector::HDetector(HCore::HCore *core):
        Threadable(Settings::detection.rate,"Detector",false,core){
...
void HDetector::loadDetector(){
    ...
    if(!yoloDetector){
        ...
        if(strcmp(Settings::net.libFile,"HNetCl") == 0){
            ...
            //yoloDetector = new ClDetector(Settings::net.configFile,Settings::net.weightFile,0);
            ...
            if(!loadLibrary(libname.c_str())){
                ...
                m_core->log(LOG_ERROR,"Make sure cuda is installed and your operating system can find cuda libraries");
                yoloDetector = new INeuralNet();
                return;
            }
...
void HDetector::benchmark(){
    ...
    for(int height = 96,width = 96;height <= 1024;height += 32, width += 32){
        ...
        m_core->log(LOG_INFO,"Benchmark %d times %dx%d total %fs avg %fms %ffps",times,width,height,(float)totalTime/1000000.0,avgTime/1000.0,fps);
    }
