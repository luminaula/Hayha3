...
namespace Util{
    ...
    void Route::addPoint(RoutePoint point){
        m_points.push_back(point);
        //HCore::log(LOG_DEBUG,"%f %f %u",m_points.back().x,m_points.back().y,m_points.back().utime);
    }
    ...
    void Route::travel(uint32_t utime){
        ...
        //HCore::log(LOG_DEBUG,"%f %f %d",m_currentLocation.x, m_currentLocation.y, m_currentLocation.utime);
    }
        ...
        while(std::getline(routeFile,line)){
            ...
            if(line.compare("[NAME]") == 0){
                if(!std::getline(routeFile,name)){
                    //HCore::log(LOG_ERROR,"%s line %d",filename.c_str(),lineNumber);
                    break;
                }
                else{
                ...
                while(std::getline(routeFile,line)){
                    ...
                    if(!(ss >> tmpPoint.x >> tmpPoint.y >> tmpPoint.utime)){
                        //HCore::log(LOG_ERROR,"%s line %s",filename.c_str(),lineNumber);
                    }
                    else{
                        //HCore::log(LOG_DEBUG,"[POINT] x = %f y = %f time = %uus %d",tmpPoint.x,tmpPoint.y,tmpPoint.utime,lineNumber);
