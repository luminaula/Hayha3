...
namespace HAPI{
    ...
    void setMouseRate(float value){
        Workers::mouseWorker->setRate(value);
    }

    void setKeyboardRate(float value){
        Workers::keyboardWorker->setRate(value);
    }
    ...
    void ignoreArea(float x0, float y0, float w,float h){
        Workers::apiworker->ignoreArea(x0,y0,w,h);
    }
    void clearIgnore(){
        Workers::apiworker->clearIgnores();
    }
    ...
    int64_t time(){
        return timeSinceStart(getCurrentTimeMicro());
    }
    ...
    void log(const char *logEntry){
        m_core->log(LOG_INFO,"API: %s",logEntry);
    }
    ...
    void clearKeys(){
        Workers::keyboardWorker->clearListen();
    }
    void listenKey(uint32_t code){
        Workers::keyboardWorker->listen(code);
    }

    bool checkKey(uint32_t code){
        return Workers::keyboardWorker->check(code);
    }

    void strictKeyboard(bool val){
        Workers::keyboardWorker->setStrict(val);
    }

    void setCaptureOffset(int x,int y, uint32_t utime){
        Workers::capture->setOffset(x,y,utime);
    }

    void setCaptureCenter(int x,int y, uint32_t utime){
        Workers::capture->setCenter(x,y,utime);
    }

    void setDetectionNetRatio(float value){
        Settings::detection.netRatio = value;
    }
    ...
    void startCurrentRoute(){
        Util::routeHandler.reset(m_currentRoute);
    }

    void travelCurrentRoute(uint32_t utime){
        Util::routeHandler.travel(m_currentRoute,utime);
    }

    Util::RoutePoint getCurrentLocation(){
        return Util::routeHandler.getRoute(m_currentRoute).location();
    }

    void startRoute(std::string name){
        Util::routeHandler.reset(name);
    }
    
    void travelRoute(std::string name,uint32_t utime){
        Util::routeHandler.travel(name,utime);
    }
    
    Util::RoutePoint getLocation(std::string name){
        return Util::routeHandler.getRoute(name).location();
    }

    std::string scriptsPath(){
        return OS::scriptPath;
    }

    std::string getWeapon(){
        return Settings::scripts.weaponName;
    }

    std::string getWeaponPath(){
        return Settings::scripts.weapon;
    }
    std::string getUtilPath(){
        return Settings::scripts.util;
    }

    void setWeapon(std::string name){
        sprintf(Settings::scripts.weaponName,"%s",name.c_str());
    }

    void sleep(uint32_t usec){
        std::this_thread::sleep_for(microseconds(usec));
    }
