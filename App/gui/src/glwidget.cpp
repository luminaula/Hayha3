...
namespace GUI{
    ...
    GLWidget::GLWidget():
            m_width(Settings::display.w),
            m_height(Settings::display.h)
    ...
    void GLWidget::paintGL(){
        ...
        if(!fb.m_presented){
            int width,height;
            width = (std::min)(fb.width,m_width);
            height = (std::min)(fb.height,m_height);
        ...
        painter.drawImage(centerX,centerY,m_image);
    ...
    DetectionWindow::DetectionWindow(MainWindow *mw):
            mainWindow(mw){
