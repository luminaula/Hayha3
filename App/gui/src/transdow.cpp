...
TransWindow::TransWindow(QWidget *parent)
    : QWidget(parent, Qt::SubWindow), m_width(Settings::screen.w), m_height(Settings::screen.h), painter(this) {
...
void TransWindow::paintEvent(QPaintEvent *) {
    ...
    if (Settings::overlay.showArea) {
        painter.setPen(penGreen);
        painter.drawRect(fb.capture.x - 1, fb.capture.y - 1, fb.capture.w + 2, fb.capture.h + 2);
    }
