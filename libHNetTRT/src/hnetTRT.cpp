...
std::vector<bbox_t> YoloNet::detect(image_t img,float thresh){
    ...
    for(auto &b : binfo){
        bbox_t baux;
        baux.x = b.box.x1 * img.detectionW/img.w + img.detectionX;
        baux.y = b.box.y1 * img.detectionH/img.h + img.detectionY;
        baux.w = (b.box.x2 - b.box.x1) * img.detectionW / img.w;
        baux.h = (b.box.y2 - b.box.y1) * img.detectionH / img.h;
        baux.prob = b.prob;
