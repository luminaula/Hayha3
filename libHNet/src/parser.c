...
convolutional_layer parse_convolutional(list *options, size_params params) {
    ...
    convolutional_layer layer = make_convolutional_layer(
        batch, h, w, c, n, size, stride, padding, activation, batch_normalize,
...
softmax_layer parse_softmax(list *options, size_params params) {
    ...
    softmax_layer layer =
        make_softmax_layer(params.batch, params.inputs, groups);
...
layer parse_yolo(list *options, size_params params) {
    ...
    if (l.outputs != params.inputs) {
        ...
        printf("filters= in the [convolutional]-layer doesn't correspond to "
               "classes= or mask= in [yolo]-layer \n");
        exit(EXIT_FAILURE);
    }
...
layer parse_region(list *options, size_params params) {
    ...
    if (l.outputs != params.inputs) {
        printf("Error: l.outputs == params.inputs \n");
        printf("filters= in the [convolutional]-layer doesn't correspond to "
               "classes= or num= in [region]-layer \n");
        exit(EXIT_FAILURE);
    }
    // assert(l.outputs == params.inputs);
...
cost_layer parse_cost(list *options, size_params params) {
    ...
    cost_layer layer =
        make_cost_layer(params.batch, params.inputs, type, scale);
...
network parse_network_cfg_custom(char *filename, int batch) {
    ...
    if (workspace_size) {
        ...
        if (gpu_index >= 0) {
            net.workspace =
                cuda_make_array(0, workspace_size / sizeof(float) + 1);
        } else {
...
void load_connected_weights(layer l, FILE *fp, int transpose) {
    ...
    if (l.batch_normalize && (!l.dontloadscales)) {
        ...
        // l.outputs), variance_array(l.scales, l.outputs));
        // printf("rolling_mean: %f mean %f variance\n",
        // mean_array(l.rolling_mean, l.outputs), variance_array(l.rolling_mean,
        // l.outputs)); printf("rolling_variance: %f mean %f variance\n",
