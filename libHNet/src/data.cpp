...
void*load_data_in_thread(load_args args)
{
    
    struct load_args *ptr = (load_args*)calloc(1, sizeof(struct load_args));
    *ptr = args;
    std::thread *thread = new std::thread(load_thread,ptr);
...
void *load_threads(void *ptr)
{
    ...
    for(i = 0; i < args.threads; ++i){
        ...
        std::thread *thread = new std::thread(load_data_in_thread,args);
...
void *load_data(load_args args)
{
    ...
    *ptr = args;
    ...
    std::thread *thread = new std::thread(load_threads,ptr);
