...
std::thread train_network_in_thread(network net, data d, float *err)
{
    ...
    return std::thread(train_thread,ptr);
}
...
std::thread sync_layer_in_thread(network *nets, int n, int j)
{
    ...
    return std::thread(sync_layer_thread,ptr);
}
...
void sync_nets(network *nets, int n, int interval)
{
    ...
    *nets[0].seen += interval * (n-1) * nets[0].batch * nets[0].subdivisions;
    ...
    for (j = 0; j < layers; ++j) {
        ...
        threads.push_back(sync_layer_in_thread(nets, n, j));
    }
    for (j = 0; j < layers; ++j) {
...
float train_networks(network *nets, int n, data d, int interval)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        threads.push_back(train_network_in_thread(nets[i], p, errors + i));
    }
    for(i = 0; i < n; ++i){
